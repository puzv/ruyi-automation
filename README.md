...
安装依赖后，需要自行准备与当前 Mac 架构匹配的 Playwright Chromium，并解压到项目内的
`.playwright-browsers/` 目录。M 系列 Mac 使用 `darwin-arm64`，Intel Mac 使用
`darwin-x64`。例如浏览器压缩包解压后应保留类似以下结构：

```text
.playwright-browsers/
└── chromium-1234/
    └── chrome-mac-arm64/       # M 系列；Intel 使用 chrome-mac-x64
```
