...
/**
 * Force release the shared Chromium profile without touching profile data.
 * The SingletonLock symlink identifies the owning PID; terminate only that
 * process (SIGTERM first, then SIGKILL after the grace period) and remove
 * lock entries only once they are confirmed stale.
 */
function forceReleaseProfile({ timeoutMs = 5000, graceMs = 1500 } = {}) {
  ...
  } catch (_) { /* no owner marker */ }

  // Avoid ever signalling this service itself if a malformed/reused lock
  // happens to contain our PID. On Unix, additionally verify the command
  // belongs to Chrome/Chromium before terminating a PID that may have been
  // reused since the lock was written.
  if (ownerPid === process.pid) ownerPid = null;
  if (ownerPid && process.platform !== "win32") {
    const probe = spawnSync("ps", ["-p", String(ownerPid), "-o", "command="], { encoding: "utf8" });
    const command = probe.stdout || "";
    // A PID can be reused after Chrome exits. Require both a Chromium binary
    // and this exact --user-data-dir marker before signalling it.
    if (probe.status !== 0 || !/(?:chrome|chromium)/i.test(command)
      || !command.includes(path.resolve(config.profileDir))) ownerPid = null;
  }
  const terminated = [];
  const signal = (pid, name) => {
    if (!Number.isInteger(pid) || pid <= 0) return false;
    try { process.kill(pid, name); terminated.push({ pid, signal: name }); return true; }
    catch (error) { return error.code === "ESRCH"; }
  };

  if (ownerPid) signal(ownerPid, "SIGTERM");
  // Keep this synchronous: callers invoke it while holding the release gate,
  // preventing another workflow from racing a profile relaunch.
  if (graceMs > 0) sleepSync(graceMs);
  let released = waitForProfileRelease({ timeoutMs });
  if (!released && ownerPid) {
    signal(ownerPid, "SIGKILL");
    released = waitForProfileRelease({ timeoutMs });
  }
  const remainingLocks = profileLockFiles();
  return { released: released && remainingLocks.length === 0, ownerPid, terminated, remainingLocks };
}
...
module.exports = { checkPreflight, profileLockFiles, waitForProfileRelease, forceReleaseProfile };
