...
前者循环 `analyseToDoList.json` 并排除已在 `done.json` 的项目；后者选择人群包、填任务名、设置固定筛选，只有 URL 跳转到指定成功页面才记录完成。平台标记人群规模小于 1000 时，直接从分析清单移除且不写入 `done.json`。

### `downloadAll.js` / `download.js`

前者按 `done.json` 循环；后者在结果页逐页查找任务，必要时搜索，先检查任务是否仍在排队/计算，再监听 Playwright 下载事件并保存文件。小于 1 KB 的下载会尝试解析 JSON，明确 `success:false` 时视为失败且不落盘，并保留清单项。
...
浏览器脚本都以 `launchPersistentContext` 使用共享 `ruyi-profile`，默认后台（无头）运行；设置 `RUYI_HEADLESS=0` 后可切换为可视调试模式，并在 `finally` 中关闭上下文。网页控制台的登录流程按 DataNexus -> 如翼串行打开页面；检测到登录窗口关闭后会复查 Cookie，再进入下一个站点。各脚本在登录后重新确认目标页面和业务元素，避免沿用已关闭或仍处于 SSO 跳转中的标签页。批处理使用 Node `spawnSync` 串行调用单项脚本，不会并发操作浏览器或清单。每个阶段都检查清单是否变短，检查失败即停止，要求人工核实网页状态。
...
- 创建或分析超时：清单不变；先在网页确认是否已创建，再决定重试。人群规模小于 1000 属于不可重试项，会从分析清单移除。
